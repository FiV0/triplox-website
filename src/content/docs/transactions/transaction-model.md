...
idents and `:db/id` sugar into the internal used Datom data structure used by the rest of the
...
The tx pipeline than checks that any explicit entity ids in the Datoms map to already allocated
...
We continue resolving tempids and upserts. This is I think the most hairy step in the tx pipeline.
...
Only after a successfull commit, does the in-memory schema get updated and the partition counters set.
